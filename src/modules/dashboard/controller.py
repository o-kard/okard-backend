...
@router.get("/campaigns", response_model=list[schema.CampaignProgress])
async def user_campaigns_progress(
    clerk_id: str = Query(...),
    limit: int = Query(20, ge=1, le=100),
    offset: int = Query(0, ge=0),
    db: Session = Depends(get_db),
):
    return await service.get_campaign_progress(db, clerk_id, limit, offset)
...
@router.get("/campaigns/trending", response_model=list[schema.TrendingCampaign])
async def get_trending_campaigns(
    day: date = date.today(),
    db: Session = Depends(get_db),
    clerk_id: str = Query(...)
):
    return await service.list_trending_campaigns(db, day, clerk_id)