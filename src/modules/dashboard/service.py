...
async def get_post_progress(db: Session, clerk_id: str, limit: int = 20, offset: int = 0) -> list[schema.PostProgress]:
    user = await get_user_by_clerk_id(db, clerk_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def list_trending_posts(db, day: date, clerk_id: str):
    user = await get_user_by_clerk_id(db, clerk_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
