...
3.  **โมเดลฐาน (Declarative Base)**: โมเดลเฉพาะของแต่ละฟีเจอร์ (User, Campaign และอื่นๆ) ทั้งหมดจะสืบทอดมาจาก `Base` ที่กำหนดไว้ใน `db.py` ซึ่งช่วยให้ SQLAlchemy สามารถสร้างโครงสร้างฐานข้อมูลแบบรวมหมู่ได้
