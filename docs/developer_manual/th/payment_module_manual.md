...
โมดูลการชำระเงินมีการทำงานร่วมกับโมดูลโพสต์ (Post) และโมดูลผู้สนับสนุน (Contributor) อย่างใกล้ชิดเพื่อให้แน่ใจถึงความสอดคล้องของข้อมูลระหว่างการทำธุรกรรม
...
- [service.py](file:///Users/wisapat/Documents/Code/Git/okard-backend/src/modules/payment/service.py): ตรรกะที่สำคัญในการจัดการธุรกรรม, การอัปเดตยอดรวมของโพสต์ และการกระตุ้นการแจ้งเตือนเมื่อสำเร็จ
...
- [model.py](file:///Users/wisapat/Documents/Code/Git/okard-backend/src/modules/payment/model.py): กำหนดตาราง `Payment` พร้อมระบุยอดเงิน, วิธีการชำระเงิน และการอ้างอิงถึงผู้ใช้/โพสต์
...
sequenceDiagram
    ...
    participant Post as โมดูล Post / ผู้สนับสนุน
    participant Notif as บริการการแจ้งเตือน

    User->>PCI: กรอกยอดเงินและรายละเอียด
    PCI->>API: POST /payment
    API->>Post: 1. เพิ่มผู้สนับสนุน / อัปเดตยอดรวมของโพสต์
    API->>Post: 2. หากถึงเป้าหมาย ให้ตั้งสถานะโพสต์เป็น SUCCESS
    API->>Notif: 3. สร้างการแจ้งเตือนเมื่อระดมทุนสำเร็จ
    API-->>PCI: ชำระเงินสำเร็จ
    PCI->>User: นำทางกลับไปยังหน้าโพสต์
```
...
| :--- | :--- | :--- | :--- |
| `create_payment` | ตรวจสอบสถานะแคมเปญ, บันทึกการชำระเงิน และอัปเดตมาตรวัดการระดมทุนของโพสต์ | `db`, `clerk_id`, `data` (Schema) | วัตถุ `Payment` |
| `calculate_backup_amounts`| (ถูกเรียกผ่าน reward_service) อัปเดตการคำนวณการให้ทุนแบบแบ่งระดับ (Tiered funding) | `db`, `post_id` | ไม่มี |
...
| โปรแกรมย่อย | หน้าที่ความรับผิดชอบ | ข้อมูลเข้า (Input) | ข้อมูลออก (Output) |
| :--- | :--- | :--- | :--- |
| `handleSubmit` | ตรวจสอบข้อตกลงในแบบฟอร์มและส่งข้อมูลไปยัง Backend | สถานะคอมโพเนนต์ (ยอดเงิน, ทิป ฯลฯ) | นำทางไปยังหน้าแสดงโพสต์ |
| `useEffect (Load Post)` | ดึงข้อมูลแคมเปญเป้าหมายเพื่อแสดงความคืบหน้าและส่วนหัว | `postId` | อัปเดตสถานะคอมโพเนนต์ |

---

## 4. การสื่อสารและพารามิเตอร์ (Communication & Parameters)

1.  **ความสมบูรณ์ของการทำธุรกรรม (Transaction Integrity)**: บริการ `create_payment` ในส่วนหลังบ้านจะรวมหลายการดำเนินการไว้ด้วยกัน (บันทึกรายรับ, อัปเดตผู้สนับสนุน, อัปเดตยอดเงินรวมของโพสต์) ซึ่งควรอยู่ในบล็อกการทำธุรกรรมฐานข้อมูล (Transaction block) เพื่อให้แน่ใจว่าข้อมูลมีความสอดคล้องกัน
2.  **รหัสอ้างอิงภายนอก**: ใช้ `clerk_id` จากฝั่ง Frontend เพื่อค้นหา `user_id` ในระบบท้องถิ่น
3.  **การเปลี่ยนสถานะ**: เมื่อการชำระเงินทำให้ `current_amount` สูงกว่า `goal_amount` บริการจะเปลี่ยนสถานะ (`state`) ของโพสต์จาก `published` เป็น `success` โดยอัตโนมัติ
