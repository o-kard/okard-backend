...
โมดูลแดชบอร์ดเป็นบริการแบบผสมผสานที่จะทำการเรียกดูข้อมูลจากโมดูล Post, Payment และ Contributor
...
- `components/`:
    ...
    - `DashboardSummary.tsx`: ตัววัดหลักระดับบนสุด (ยอดรวมที่ระดมได้, จำนวนนักลงทุน, โพสต์ที่ยังเปิดรับทุนอยู่)
    - `DashboardPosts.tsx`: มุมมองแบบตารางของความคืบหน้าแคมเปญในแต่ละรายการ
...
| `get_post_progress` | คำนวณเปอร์เซ็นต์การระดมทุนและจำนวนนักลงทุนต่อโครงการ | `db`, `clerk_id` | `List[PostProgress]` |
...
4.  **การแบ่งหน้า (Pagination)**: รายการโครงการในแดชบอร์ดรองรับ `limit` และ `offset` เพื่อให้จัดการกับผู้ใช้ที่มีแคมเปญจำนวนมากได้อย่างมีประสิทธิภาพ
